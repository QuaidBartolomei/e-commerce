...
export const ImageGalleryProvider: FC<Props> = props => {
  const { imageUrls, children } = props;
...
export function useImageGalleryState() {
  ...
  if (context === undefined) {
    throw new Error('useImageGalleryState must be used within a UserProvider');
  }
  return context;
}
...
export function useImageGalleryDispatch() {
  ...
  if (context === undefined) {
    throw new Error('useImageGalleryState must be used within a CountProvider');
  }
  return context;
}
